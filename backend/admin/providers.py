...
class EmailPasswordProvider(Provider):
    name = "login_provider"
    ...
    def __init__(
            self,
            admin_model: Type[AbstractEmailAdmin],
            login_path="/login",
            logout_path="/logout",
    ...
    async def pre_save_admin(self, _, instance: AbstractEmailAdmin, using_db, update_fields):
        if instance.pk:
    ...
    async def password(
            ...
            admin: AbstractEmailAdmin = Depends(get_current_admin),
            resources=Depends(get_resources),
    ):
        error = None
